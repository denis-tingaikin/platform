...
export async function record (options: FileUploadOptions): Promise<void> {
  ...
  showPopup(Recorder, {}, undefined, async (status: any) => {
    console.log(status)
...
async function uploadRecording (recordingId: string, onUploaded: FileUploadCallback): Promise<void> {
  let uploadUrl: string = ''
  try {
    uploadUrl = new URL(getMetadata(presentation.metadata.UploadURL) ?? '').origin
  } catch {
