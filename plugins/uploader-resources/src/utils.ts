...
import { type FileUploadOptions, type FileWithPath, toFileWithPath, type UploadMethodExtension } from '@hcengineering/uploader'
...
export async function getUploadMethods (): Promise<UploadMethodExtension[]> {
  const client = getClient()
  return await client.findAll<UploadMethodExtension>(uploader.class.UploadMethodExtension, { extension: uploader.extension.UploadExtensionId })
}
...
export async function uploadFiles (files: File[] | FileList, options: FileUploadOptions): Promise<void> {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    void limiter.add(async () => { await uploadFile(data, { name: files[i].name, uuid, relativePath }, options) })
  }

  await limiter.waitProcessing()
}

async function uploadFile (file: FileWithPath, metadata: Record<string, any>, options: FileUploadOptions): Promise<void> {
