...
import type { FileUploadOptions, FileUploadPopupOptions, FileWithPath, UploadMethodExtension } from './types'
...
export async function getUploadMethods (): Promise<UploadMethodExtension[]> {
  const fn = await getResource(uploader.function.GetUploadMethods)
  return await fn()
}
